...
public class Main {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try
        {
            ...
            solve = ady.topologicalOrder();
            for(int i =0;i<solve.size();i++)
            {
                writer.print(solve.get(i)+1 +" ");
            }
        }
...
class AdjacencyList implements Graph {
    ...
    */
    ...
    @Override
    public long[] DAG_SP(String originVertex) throws CycleDetectedException, VertexNotFoundException {
        int sourceId = search(originVertex);
        long[] dist = new long[vertexCount];
    ...
    public long[] countPathsInDAG(String originVertex) throws VertexNotFoundException, CycleDetectedException {
        ...
        /* In aristasEnt, we are going to store, for each node, a list of the nodes from
        which it can be reached. It is not necessary to store the complete edge */
    ...
    @Override
    public long minimumSpanningTreeKruskal() {
        ...
        for(EdgeExt edge: EdgeList){
            int conjIni = S.Find(edge.getStart());
            int conjDest = S.Find(edge.getDestination());
...
interface Graph {
    ...
    */
     ...
     * @throws VertexNotFoundException
     */
    long[] DAG_SP(String originVertex) throws CycleDetectedException, VertexNotFoundException;
     ...
     */
    long minimumSpanningTreeKruskal();
}