...
// Link : https://dmoj.uclv.edu.cu/problem/moocast
...
public class Main {
    ...
    public static void main(String args[]) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            for(int j = i+1;j<n;j++)
            {
                long dist = (x_value.get(j)-x_value.get(i));
                dist = dist * dist;
                long dist2 = (y_value.get(j)-y_value.get(i));
                dist2 = dist2 * dist2; 
                graph.get(i).add(new Edge(j,(dist+dist2)));
                graph.get(j).add(new Edge(i,(dist+dist2))); 
            }
        }
        ...
        try
        {
            long sol = ady.minimumSpanningTreeKruskal();
            writer.print(sol);
    ...
    */
    ...
    public long minimumSpanningTreeKruskal() throws GraphNotConnectedException {
        ...
        for(EdgeExt edge: EdgeList){
            ...
            if( conjIni != conjDest ){
                mst = Math.max(mst,edge.getWeight());
